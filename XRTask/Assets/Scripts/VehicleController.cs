...
public class VehicleController : MonoBehaviour
{
    ...
    private void Move(double inputManagerForwardVal, double inputManagerSidesVal)
    {
        ...
        float stearVal = Mathf.Clamp((float)inputManagerSidesVal, -1, 1)*acceleration;

        float WheelRotAngle = acceleration * _steerRotBonus/0.5f;
        _targetWheelRot = Quaternion.AngleAxis(WheelRotAngle * Time.deltaTime, Vector3.right);

        float steringAngle = stearVal * _maxSteerAngle;
        _targetRot *= Quaternion.AngleAxis(steringAngle*2 * Time.deltaTime, Vector3.up);
        ...
        //vehicle rotation
        transform.rotation *=Quaternion.AngleAxis(steringAngle * Time.deltaTime, Vector3.up);
        ...
        //wheel rotation
        Debug.Log(Math.Abs(_targetWheelRot.x));
        if (Mathf.Abs(_targetWheelRot.x) < 0.01 * _maxSpeed)
        {
             for (int i = 0; i < _numOfWheels; i++)
            {
